package com.tencent.fge.engine.graphic.fx
{
	...
	public class Lightning extends GraphicsNode
	{
		private const SMOOTH_COLOR:uint=0x808080;
		
		private var holder:GraphicsNode;
		...
		public function Lightning(graphics:*, pcolor:uint=0xffffff, pthickness:Number=2, pgeneration:uint=0)
		{
			super(graphics);
		...
		private function init():void 
		{
			...
			holder=new GraphicsNode(this.graphics);
			...
			if(generation<childrenMaxGenerations && childrenArray.length<childrenMaxCount) 
			{
				...
				if(targetChildSteps>=2) 
				{
					for(var i:uint=0; i<n; i++) 
					{
						...
						var child:Lightning=new Lightning(this.graphics, color, thickness, generation+1);
		...
		public function draw():void 
		{
			...
			for (var i:uint=0; i<steps; i++) 
			{
				...
				//tx=startX+dx/(steps-1)*i + soffx;
				//ty=startY+dy/(steps-1)*i - soffy;
