package slicol.common
{
	...
	public class Input
	{
		private static var ms_me:Input;
		
		public static function init(stageMc:Sprite):void
		{
			ms_me = new Input(stageMc);
		}
		...
		public function Input(stageMc:Sprite)
		{
			...
			// add key listeners
			stageMc.stage.addEventListener(KeyboardEvent.KEY_DOWN, onKeyPress, false, 0, true);
			stageMc.stage.addEventListener(KeyboardEvent.KEY_UP, onKeyRelease, false, 0, true);		
			
			// mouse listeners
			if(Multitouch.inputMode == MultitouchInputMode.TOUCH_POINT)
			{
				stageMc.stage.addEventListener(TouchEvent.TOUCH_BEGIN, onMousePress, false, 0, true);
				stageMc.stage.addEventListener(TouchEvent.TOUCH_TAP, onMouseRelease, false, 0, true);
				stageMc.stage.addEventListener(TouchEvent.TOUCH_MOVE, onMouseMove, false, 0, true);
				stageMc.stage.addEventListener(TouchEvent.TOUCH_OUT, onMouseLeave, false, 0, true);		
			}
			else
			{
				stageMc.stage.addEventListener(MouseEvent.MOUSE_DOWN, onMousePress, false, 0, true);
				stageMc.stage.addEventListener(MouseEvent.CLICK, onMouseRelease, false, 0, true);
				stageMc.stage.addEventListener(MouseEvent.MOUSE_MOVE, onMouseMove, false, 0, true);
				stageMc.stage.addEventListener(Event.MOUSE_LEAVE, onMouseLeave, false, 0, true);				
			}
		...
		private function onMouseMove(e:*):void
		{
			...
			mouseX = e.stageX - inputStage.x;
			mouseY = e.stageY - inputStage.y;
			// Store offset
		...
		// stage
		static public var inputStage:Sprite;
	}
